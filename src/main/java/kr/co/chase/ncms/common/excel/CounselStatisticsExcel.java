...
public class CounselStatisticsExcel extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook) throws Exception{
		...
		for(int i=0; i < cslInfo.size(); i++) {
			...
			this.cellStyleLoop(40, 40, contentCellStyle, row, cell, StringUtils.defaultIfEmpty((String)cslInfo.get(i).get("CSL_CTNT"), ""));
			this.cellStyleLoop(41, 41, contentCellStyle, row, cell, StringUtils.defaultIfEmpty((String)cslInfo.get(i).get("CSL_RST"), ""));
		}
	}
