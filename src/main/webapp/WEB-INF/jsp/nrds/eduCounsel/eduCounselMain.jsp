...
<script type="text/javaScript" language="javascript" defer="defer">
	$(document).ready(function(){
		...
		setMemInfo = function(mbrNo){
			$.ajax({
				...
				success : function(json){
					...
					if(info.MBR_NO != ""){
						...
						//$("input[name='mngUsrId']").val(info.MNG_USR_NM);
					}
				},
				error : function(xhr, status){
					console.log(xhr);
			...
			getCslCureList(mbrNo);

			$("button#excelButNo").hide();
			$("button#excelButYes").show();
		},
		<%-- 교육상담 상담 이력 조회 --%>
		getCslCureList = function(mbrNo){
			$.ajax({
				url : '/nrds/getClsCureList.do',
				type : 'POST',
				data : {
					mbrNo : mbrNo
				},
						...
						if(res.clsCureList.length > 0){
							...
							$(res.clsCureList).each(function(idx, obj){
								inHtml = inHtml
									   ...
									   + "	<td><div class='cell'>" + obj.CSL_MNG_NM + "</div></td>"
									   + "	<td><div class='cell'>"
			...
			$.ajax({
				...
				success : function(res){
					if(res.err != "Y"){
						...
						$("input[name='cslNm']").val(dataInfo.CSL_MNG_NM);
						...
						$("input[name='cslDt2']").val(formatDate(dataInfo.CSL_DT));
						$("input[name='cslFmTm2']").val(dataInfo.CSL_FM_TM);
						$("input[name='cslToTm2']").val(dataInfo.CSL_TO_TM);
						$("input[name='cslTermTm2']").val(dataInfo.CSL_TERM_TM);
						...
						if(termTm > 0){
							...
							$("span#cslTermTm2").text(termTm);
							$("input[name='cslTermTm']").val(termTm);
							$("input[name='cslTermTm2']").val(termTm);
						}
		...
		<%-- 엑셀다운로드 --%>
		eduCounselExel = function(){
			if($("input[name='cslNo']").val() != ""){
				$("#excelForm").append("<input type='hidden' name='cslNo' value='" + $("input[name='cslNo']").val() + "' />");
				$("#excelForm").attr("action", "/nrds/eduCounselDownload.do");
				$("#excelForm").submit();
			}else{
				alert("접수번호를 선택 하세요.");
			}
...
<!-- 페이지 타이틀 -->
<div class="tit-area">
	<h1><i class="el-icon-s-order" style="color: rgb(0, 108, 185);"></i> 교육상담</h1>
</div>
<!-- // 페이지 타이틀 -->
<!-- 상단 버튼 -->
<div class="top-right-btn">
	...
	<button type="button" onclick="javaScript:eduCounselExel();" id="excelButYes" class="el-button normal el-button--default el-button--small is-plain" style="display: none;">
		<i class="el-icon-document"></i><span>엑셀다운로드</span>
	</button>
</div>
<!-- // 상단 버튼 -->
	...
	<div class="section">
		<table class="w-auto">
			...
			<tr>
				...
				<th>상담사</th>
				<td>
					<div class="dsp-ibk"><input type="text" name="" value="<c:out value="${LoginSiteNm }"/>" class="el-input__inner" readonly placeholder="지역센터" style="width: 130px;" /></div>
				</td>
				<td>
					<div class="dsp-ibk"><input type="text" name="cslMngNm" value="<c:out value="${LoginUserNm}" />" class="el-input__inner" readonly placeholder="상담사" style="width: 130px;" /></div>
					<div class="dsp-ibk is-disabled"><input type="hidden" name="cslMngUsrId" value="<c:out value="${LoginUserId}" />" class="el-input__inner" readonly style="width: 130px;" /></div>
				</td>
			</tr>
			</tbody>
		</table>
				...
				<div class="row">
					<table class="w-auto wr-form">
						...
						<tr>
							<th>담당자</th>
							<td>
								<div class="dsp-ibk"><input type="text" name="cslNm" value="<c:out value="${LoginUserNm}" />" class="el-input__inner" readonly style="width: 120px;" /></div>
							</td>
							<th><span class="required">*</span> 상담일시</th>
							<td>
								<div class="dat-pk">
